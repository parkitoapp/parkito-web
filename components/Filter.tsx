    ...
    <>
      ...
      <div id="filters" className="flex flex-row gap-4 mt-10 mb-4 px-4 overflow-x-auto w-full md:w-[50%] mx-auto">
        <Button
          ...
          className={`px-2 text-lg rounded-full hover:cursor-pointer ${selectedFilter === null ? 'bg-accent text-accent-foreground' : ''
            }`}
        >
          {allItemsLabel}
        </Button>
        ...
        {filterOptions.map((option, idx) => (
          <Button
            ...
            className={`px-2 text-lg rounded-full hover:cursor-pointer ${selectedFilter === option ? 'bg-accent text-accent-foreground' : ''
              }`}
          >
            {option}
          </Button>
