...
export type LoopItem = { node: React.ReactNode; key?: string | number };
...
export interface LogoLoopProps {
  ...
  renderItem?: (item: LoopItem, key: React.Key) => React.ReactNode;
  ariaLabel?: string;
  className?: string;
...
export const LogoLoop = React.memo<LogoLoopProps>(
  ({
    ...
    const renderLoopItem = useCallback(
      (item: LoopItem, key: React.Key) => {
        if (renderItem) {
          return (
            <li
              className={cx(
                'text-[length:var(--logoloop-logoHeight)] py-10 leading-[1]',
        ...
        return (
          <li
            className={cx(
              'text-[length:var(--logoloop-logoHeight)] leading-[1]',
              isVertical ? 'mb-[var(--logoloop-gap)]' : 'mr-[var(--logoloop-gap)]',
              scaleOnHover && 'overflow-visible group/item'
            )}
            key={key}
    ...
    const logoLists = useMemo(
      () =>
        Array.from({ length: copyCount }, (_, copyIndex) => (
          ...
          >
            {logos.map((item, itemIndex) => renderLoopItem(item, `${copyIndex}-${itemIndex}`))}
          </ul>
        )),
      [copyCount, logos, renderLoopItem, isVertical]
    );
