 ...
 */
async function fetchCityImageFromSupabase(citySlug: string): Promise<string | null> {
    try {
        ...
        const fileExists = listData?.some(file => file.name === `${citySlug}.webp`);
...
const getCitiesCached = CACHE_DURATION === 0
    ? async () => {
        ...
        const imageMap = new Map(cityImages.map(({ slug, supabaseImageUrl }) => [slug, supabaseImageUrl]));
